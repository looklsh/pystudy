...
class Point:
    ...
    # 클래스 이름 공간 내에 생성
    # 모든 인스턴스 멤버 공유
    # 클래스 멤버는 인스턴스 생성 없이도 사용 할 수 있다
    instance_count = 0

    def __init__(self, x = 0, y = 0): # 생성자
        #파이썬은 여러개의 생성자를 만들 수 없으므로 범용적으로 사용될 수 있는 유일한 생성자를 작성
        self.x = x
        self.y = y
    ...
    def __str__(self): # 문자열 출력
        # str() 호출 혹은 print를 할 때 사용되는 비공식 문자열(일반사용자 대상)
        return "Point x={}, y={}".format(self.x, self.y)

    def __repr__(self): # 문자열 출력
        # 개발자용, 공식문자열
        # repr() 함수로 전달 받을 수 있다
        # 이 문자열로 해당 객체를 복원해 낼수 있어야 한다
        return "Point(x={}, y={})".format(self.x, self.y)
    ...
    # 연산자 오버로딩
    # 새로운 데이터 타입에 필요한 연산자의 행동을 재정의 하는 것
    # 산술 연산자 오버로딩 예제
...
def bound_class_method():
    ...
    p = Point()
    # bound 방식의 경우 첫번째 인자 self는 전달하지 않아도 됨
    p.setX(10)
    p.setY(20)
...
def str_repr():
    ...
    # 이 때 repr로 전달 받은 문자열(개발자용)을 넘겨주면
    # 같은 객체가 복원되야 한다
    p_repr = eval(repr(p))
