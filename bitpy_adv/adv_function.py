...
def scope_func3(a):
    ...
    # 어디에서 글로벌 객체가 변경 되었는지 추적이 힘들기 때문
    x = 3
...
# 값을 return 하지 않았거나 끝날 때까지 아무 return도 없는 경우 -> None(Java의 null과 비슷)
def times(a, b):
    return a * b

#함수 자체도 객체로 판단
# 다른 식별자에 할당 할 수도 있고 다른 함수의 인수로 전달할 수 있다
fun = times
print("fun:", type(fun))
# 객체가 호출가능한 객체인지(함수) 판별하려면 callable 함수
...
# 인수의 기본값
def incr(a, step=1): # 두 번째 인자값은 1이 기본 값이다
    return a + step

print(incr(10)) # 두번째 인자 step이 기본값을 가지고 있으므로 기본 값으로 세팅
print(incr(10, 2)) # 기본값을 무시하고 새로운 값을 할당

# 기본적으로 파이썬은 인수의 이름을 명시, 인자 전달 할 수 있다
# 인자의 순서가 중요하지 않게 된다

print(incr(step=3, a=10))

#가변 인수
# 정해지지 않은 개수의 인자를 받을 때 가변 인자를 받을 변수의 앞에 * 명시
# -> 순차 자료형으로 변환되어 입력
...
def func_args(a, b, *args, **kwd):
    # a, b는 고정인자: 반드시 넘겨줘야 함
    # 그 뒤에 남은 인자의 목록은 aargs로 넘어 올 것(tuple)
    # 그 뒤에 남은 인자는 키워드(dict)
    print("고정인자:", a, b)
    print("가변인자:", args)
    print("키워드인자:", kwd)

    if "option1" in kwd:
        print("option1이 {}로 설정 되었습니다".format(kwd['option1']))
    else:
...
# 1~20까지 수열을 4로 나누었을 때의 나머지 순으로 정렬
