...
def write01():
    ...
    f.close() # 파일 사용후 반드시 닫아줌

def write02():
    # append 모드: 이미 파일이 있으면 내용은 그대로 두고 뒷쪽에 새로운 데이터를 추가
    f = open("./sample/test.txt", "a", encoding="utf-8")
    f.write("Append 모드로 저장하였습니다")
    f.close()

def copy_binary():
    # 2진 데이터 파일을 다룰 때는 파일 형식을 b로 설정
...
def read03():
    # 한 번에 읽어와서 줄 단위로 리스트 생성: readlines
    f = open("./sample/sangbuk.csv", "rt", encoding="utf-8")
    lines = f.readlines()
...
def safe_open():
    ...
    # with~ as 를 사용하면 해당 블록이 끝났을 때 자동으로 close
    with open("./sample/test.txt", "rt", encoding="utf-8") as f:
        print(f.read())
        # close 해주지 않아도 됨

#pickle의 사용
# 파이썬 객체의 직렬화 -> 역직렬화를 위한 모듈
...
def pickle_dump():
    # 직렬화: dump
    # 사전 객체를 생성 -> 직렬화
    with open("./sample/players.bin", "wb") as f: #반드시 binary 모드여야 함
        data = {"baseball": 9}
        pickle.dump(data, f)
        # 필요할 시는 3번째 인자로 pickle protocol의 버전을 명시

def pickle_load():
    # 역직렬화: load
    # load의 경우 dump시 프로토콜 버전은 명시했어도 load시에는 명시할 필요없음
    print("현재의 pickle protocol:", pickle.HIGHEST_PROTOCOL)
    # players.bin 에서 pickle ㄱ개체 역직렬화
...
def pickle_dump_multi():
    ...
    with open("./sample/players.bin", "wb") as f:
        ...
        pickle.dump({"basetball": 5}, f, pickle.HIGHEST_PROTOCOL)
...
def example():
    # sangbuk.csv -> 불러와서 dict와 list를 생성
    # pickle로 저장
    players = []
