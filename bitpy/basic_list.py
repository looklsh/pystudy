...
def define_list():
    ...
    #다른 순차 자료형을 list() 객체함수로 list로 변경할 수 있다
...
def list_oper():
    ...
    print(copy[3][1])
    del copy[3]
    #extend는 인자로 부여받은 리스트를 뒤에 연결(확장)
    copy.extend([3, 4])
    print("EXTEND:", copy)

    #insert(i, x): i인덱스에 x를 추가
    copy.insert(2, [1, 2, 3]) #2번 인덱스에 [1, 2, 3]
    print("INSERT:", copy)
    ...
    #슬라이싱을 이용, 가장 마지막에 삽입(extend와 비슷)
    lst2[len(lst2):] = [30]
    print("lst2:", lst2)
    #슬라이싱을 이용, 가장 앞쪽에 삽입
    lst2[:0] = [40]
...
def list_method():
    ...
    # - sort: 객체내부의 정렬 메서드 -> 객체 내부의 내용을 변경
    # sort, sorted는 그 이외의 사용방법은 동일하다
    print("Sorted 이전:", copy)
    print("Sorted 수행:", sorted(copy)) #기본적으로 오름차순 정렬
    ...
    #역순 정렬: sort, sorted 에 reverse=True를 부여하면 역순 정렬
    copy = lst.copy()
    print("역순 정렬 이전:", copy)
...
def loop():
    ...
    #순차형 자료형은 for...in...문으로 차례대로 요소를 반환 받을 수 있다(별도의 인덱스 변수는 없다)
    for word in words:
...
def stack_ex():
    """
    리스트를 활용한 stack구현
    append와 pop메서드를 이용하면 stack 자료형을 구현할 수 있다
    """
    stack = []
...
def queue_ex():
    """
    리스트를 활용한 Queue 자료형의 구현
    리스트의 append, pop(0)을 이용하면 Queue구현 가능
    """
    queue = []
    queue.append(1)
    queue.append(2)
