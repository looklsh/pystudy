#.py파일 한 개는 하나의 모듈이다
#파일명 자체가 모듈의 이름이 되므로 파일명 정할 때도 변수 명명규칙을 위반하지 않도록 함
...
#def arith_oper():
    #블록은 시작됐지만 구현부가 없을 경우
    #pass

def arith_oper():
    print("=====산술연산")
    #+,-,*,/
    #/: 정수와 정수 나눗셈이더라고 소숫점이 발생하면 실수로 변경
    print(7/5) #정수를 정수로 나눠도 실수 형태
    print(7//5) #나눗셈의 몫
    print(7%5) #나눗셈의 나머지
...
def variable_ex():
    print("======변수")
    #파이썬은 변수는 선업 작업이 없고 최초할당시 변수가 선언된다
    #동적 인터렉티브 언어이기 때문에 타입 지정되지 않는다(할당시에 타입이 결정)
    #변수명은 문자,숫자, _의 조합
    #숫자로 시작할 수 없다
    #예약어는 사용할 수 없다
    ...
    a = 1
    #여러 변수를 한꺼번에 할당
    c, d = 3.5, 5.3 #변수의 개수와 값의 개수는 일치해야한다
    print(c, d)
    #같은 값을 여러 변수에 동시할당
    ...
    print("a는 str객체?", isinstance(a, str))
    n = 3.14
    #변수가 가리키는 객체의 타입을 비교할 때  여러개의 타입 중 하나인지 확인
